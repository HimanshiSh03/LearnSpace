...
const Leaderboard = () => {
  ...
  const getContributorRole = (points) => {
    if (points >= 100) return { name: "Master", color: "text-purple-600", bgColor: "bg-purple-100" };
    if (points >= 50) return { name: "Senior", color: "text-blue-600", bgColor: "bg-blue-100" };
    if (points >= 20) return { name: "Regular", color: "text-green-600", bgColor: "bg-green-100" };
    return { name: "Newbie", color: "text-yellow-600", bgColor: "bg-yellow-100" };
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
              ...
              {filteredContributors.map((contributor, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${role.bgColor} ${role.color}`}>
                        {role.name}
                      </span>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap">
      ...
      <div className="mt-6 pt-4 border-t border-gray-200">
        <h3 className="text-sm font-medium text-gray-900 mb-2">Point System</h3>
        <div className="flex flex-wrap gap-4 text-sm text-gray-600">
          <div className="flex items-center">
            <Star className="w-4 h-4 mr-1 text-yellow-500" />
            <span>Level 1 Task: 2 points</span>
          </div>
          <div className="flex items-center">
            <Star className="w-4 h-4 mr-1 text-blue-500" />
            <span>Level 2 Task: 5 points</span>
          </div>
          <div className="flex items-center">
            <Star className="w-4 h-4 mr-1 text-purple-500" />
            <span>Level 3 Task: 11 points</span>
          </div>
        </div>
      </div>
    </div>
